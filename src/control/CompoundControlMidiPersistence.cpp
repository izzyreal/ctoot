...
{
    auto providerId = c->getProviderId();
    auto moduleId = c->getId();
    ...
    try {
        ...
        for (auto i = 0; i < npc(track)->size(); i++) {
            ...
            auto control = c->deepFind(::automation::ControlSysexMsg::getControlId(msg));
            if(control == nullptr)
                continue;
...
{
	/*
	return ::stringBuilder().append(c->getPersistenceDomain())->append(::java::io::File::separator())
        ->append(c->getProviderId())
